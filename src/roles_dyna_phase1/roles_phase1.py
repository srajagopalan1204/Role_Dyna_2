...
from dateutil import tz
    ...
    # ---- Helpers ----
EXPECTED_HEADERS = [
        "Location", "Status", "Postion/Title", "Position/Title",
        "Primary Functional Group", "Secondary Functional Group",
        "Primary Specialization", "Secondary Specialization", "Name"
    ]

REJECT_ROW_PREFIXES = ["Planning on Retiring", "Termed", "New Hires"]

def tz_now_str(tz_name="America/New_York"):
...
def detect_header_row(df):
        """
        Return (header_row_index, renamed_columns)
        """
        for idx, row in df.iterrows():
            vals = [str(x).strip().lower() for x in row.fillna("").tolist()]
            # Check if required headers are present in this row
            needed = ["location", "status", "primary functional group", "secondary functional group"]
            ok = all(any(h == v for v in vals) for h in needed)
            if ok and ("postion/title" in vals or "position/title" in vals):
                # Construct columns from this row
                cols = [str(x).strip() for x in row.tolist()]
                return idx, cols
        return None, None

def slice_table_from_header(df, header_idx, cols):
...
def filter_reject_rows(df):
        # A row is rejected if its first non-null cell *starts with* the banned tokens (case-insensitive)
        def starts_with_banned(row):
            for v in row:
...
def make_title_normalizer(tn_df):
        # Accept either columns: ['raw','std'] or ['Postion/Title','Position/Title']
        ...
        if not raw_col or not std_col:
            return {}
        mapping = dict(zip(tn_df[raw_col].fillna("").str.strip(), tn_df[std_col].fillna("").str.strip()))
        return mapping

def map_with_errors(series, mapping, field, tab, errors):
        def f(x):
            key = str(x).strip()
...
def build_hndl_key(df):
        ...
        for col in ["Position/Title","Primary Functional Group","Secondary Functional Group","Primary Specialization","Secondary Specialization"]:
            if col not in df.columns:
                df[col] = ""
        return (df["Position/Title"].fillna("").str.strip() + " | " +
                df["Primary Functional Group"].fillna("").str.strip() + " | " +
                df["Secondary Functional Group"].fillna("").str.strip() + " | " +
                df["Primary Specialization"].fillna("").str.strip() + " | " +
                df["Secondary Specialization"].fillna("").str.strip())

def highlight_errors_openpyxl(wb, sheet_name):
...
def main():
        ...
        tz_name = args.__dict__.get("timezone") or cfg.get("timezone") or "America/New_York"
        ...
        Oper_list_saso = var_sheets.get("Oper_list_saso") or var_sheets.get("Oper_List_Saso")
        ...
        if PFGC_Conv is not None:
            ...
            pfg_map = dict(zip(PFGC_Conv[long_col].fillna("").str.strip(), PFGC_Conv[short_col].fillna("").str.strip()))
        ...
        if PSP_Conv is not None:
            ...
            psp_map = dict(zip(PSP_Conv[long_col].fillna("").str.strip(), PSP_Conv[short_col].fillna("").str.strip()))
        ...
        if SSPL_Conv is not None:
            ...
            sspl_map = dict(zip(SSPL_Conv[long_col].fillna("").str.strip(), SSPL_Conv[short_col].fillna("").str.strip()))

        # Title normalizer
        title_map = make_title_normalizer(Title_Normalization) if Title_Normalization is not None else {}

        # Operator map (Name -> oper)
        oper_map = {}
        if Oper_list_saso is not None:
            df_op = normalize_cols(Oper_list_saso)
            # Attempt best-guess columns
            cols = {c.lower(): c for c in df_op.columns}
            name_col = cols.get("name") or cols.get("operator name") or list(df_op.columns)[0]
            oper_col = cols.get("operator") or cols.get("role name") or list(df_op.columns)[0]
            if name_col and oper_col and name_col != oper_col:
                oper_map = dict(zip(df_op[name_col].fillna("").str.strip(), df_op[oper_col].fillna("").str.strip()))
        ...
        if Loc_Sz is not None:
            ...
            loc_size_map = dict(zip(df_ls[loc_col].fillna("").str.strip(), df_ls[size_col].fillna("").str.strip()))
        ...
        # Read RAW input (all sheets, stitch tables)
        ...
        # If FGP filter provided, allow both long & short matches
        if fgp_filter:
            fgp_lower = fgp_filter.strip().lower()
            raw_df = raw_df[
                raw_df["Primary Functional Group"].str.lower().eq(fgp_lower) |
                raw_df["Primary Functional Group"].map(lambda x: pfg_map.get(str(x).strip(), "")).str.lower().eq(fgp_lower)
            ]

        # Normalize Position/Title via Title_Normalization
        errors = []
        if title_map:
        ...
        # ADDL Keys columns (Phase 1 placeholders)
        for k in addl_keys:
            if k not in raw_df.columns:
                raw_df[k] = ""  # to be populated in Phase 2
        ...
        # Use openpyxl engine to allow post-format edits
