...
def main():
    parser = argparse.ArgumentParser(description="Roles Dyna - Phase 1 (v1.3b)")
    parser.add_argument("--config", default="config.json")
    parser.add_argument("--raw", default=None)
    parser.add_argument("--vars", default=None)
    parser.add_argument("--fgp", default="")
    parser.add_argument("--errors-only", action="store_true")
    parser.add_argument("--version", default="v_1_3b")
    ...
    mapping_errors = []
    # Capture mapping errors from earlier steps
    # (the list 'errors' was used during mapping calls)
    ...
    # Conflicts tab(s) for handler duplicates
    conflict_tabs = {}
    for tab, dfc in per_tab_conflicts.items():
        if isinstance(dfc, pd.DataFrame) and not dfc.empty:
            conflict_tabs[f"Conflicts_{tab}"] = dfc
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        for tab, df_tab in conflict_tabs.items():
            sheet_name = tab[:31]
            df_tab.to_excel(writer, index=False, sheet_name=sheet_name)
