...
def df_with_totals_by_pfg(master_df: pd.DataFrame) -> pd.DataFrame:
    """Return a simple 'Count by PFG' table with a Grand Total row."""
    ...
    grp = (master_df
           ...
           .sort_values("Primary Functional Group", key=lambda s: s.astype(str)))
...
def df_with_totals_by_pfg_role(master_df: pd.DataFrame) -> pd.DataFrame:
    """Return a block-style table: rows for each PFG's roles and a 'Total' line per PFG."""
    ...
    base = (master_df
            ...
            .size()
            .reset_index(name="Count")
            .sort_values(needed_cols, key=lambda s: s.astype(str)))
    # Build blocks with Total for each PFG
    out_rows = []
    for pfg, sub in base.groupby("Primary Functional Group", dropna=False, sort=False):
        # Keep order by ROLE_Nm
        for _, r in sub.sort_values("ROLE_Nm", key=lambda s: s.astype(str)).iterrows():
            out_rows.append({
...
def main():
    parser = argparse.ArgumentParser(description="Roles Dyna - Phase 1 (v1.4_2019, no-operator)")
    parser.add_argument("--config", default="config.json")
    parser.add_argument("--raw", default=None)
    parser.add_argument("--vars", default=None)
    parser.add_argument("--fgp", default="")
    parser.add_argument("--errors-only", action="store_true")
    parser.add_argument("--version", default="v_1_4_2019")
    ...
    # ----- Build Summary tables for Excel 2019 -----
    sum_by_pfg = df_with_totals_by_pfg(master_df)
    sum_by_pfg_role = df_with_totals_by_pfg_role(master_df)
