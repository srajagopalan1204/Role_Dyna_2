...
# Row-cleaning thresholds
HEADER_FRONT_MATCH_THRESHOLD = 3   # how many of the first columns must match header names to consider it a duplicate header row
MIN_NONEMPTY_PER_ROW = 3           # drop rows with <= this many non-empty cells
MIN_CORE_NONEMPTY = 2              # drop rows if fewer than this many "core" fields are present
...
# ---- New: remove duplicate header rows and sparse "spacer" lines ----
def is_duplicate_header_row(row, columns_lower):
    """Heuristic: if the first N cells match column names, treat as a repeated header."""
    vals = [str(x).strip().lower() for x in row.fillna("").tolist()]
    front = vals[:min(len(columns_lower), 8)]
    matches = sum(1 for i, v in enumerate(front) if v == columns_lower[i])
    # Also consider if many tokens are column names anywhere in the row
    set_cols = set(columns_lower)
    anywhere_hits = sum(1 for v in vals if v in set_cols)
    return (matches >= HEADER_FRONT_MATCH_THRESHOLD) or (anywhere_hits >= max(4, HEADER_FRONT_MATCH_THRESHOLD + 1))

def drop_duplicate_headers_and_sparse(df: pd.DataFrame) -> pd.DataFrame:
    cols_lower = [c.strip().lower() for c in df.columns]
    core = [c for c in CORE_FIELDS if c in df.columns]
    keep_idx = []
    for idx, row in df.iterrows():
        # drop duplicate header lines
        if is_duplicate_header_row(row, cols_lower):
            continue
        # drop rows that are mostly empty / separators
        row_vals = [str(x).strip() for x in row.fillna("").tolist()]
        nonempty_total = sum(1 for v in row_vals if v != "")
        if nonempty_total <= MIN_NONEMPTY_PER_ROW:
            core_nonempty = sum(1 for c in core if str(row.get(c, "")).strip() != "")
            if core_nonempty < MIN_CORE_NONEMPTY:
                continue
        keep_idx.append(idx)
...
# ---- Operator matching helpers (from v1.1) ----
...
# ---- Main ----
def main():
    parser = argparse.ArgumentParser(description="Roles Dyna - Phase 1 (v1.1b header-fix)")
    ...
    # Safe lookup without boolean "or"
    ...
    for sh_name, df in raw_book.items():
        ...
        # Remove reject-prefixed rows first
        tbl, rejected = filter_reject_rows(tbl)
        # NEW: drop duplicate header rows and sparse spacer lines
        tbl = drop_duplicate_headers_and_sparse(tbl)
    ...
    oper_opts = cfg.get("operator_match", {})
    oper_lookup = oper_lookup or {}
    raw_df["oper"] = ""
    raw_df["oper_match_method"] = ""
    if oper_lookup and "Name" in raw_df.columns:
        for i, nm in raw_df["Name"].astype(str).items():
